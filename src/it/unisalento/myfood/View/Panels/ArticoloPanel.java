...
import it.unisalento.myfood.View.Decorator.Icon.WiewIconDecorator;
import it.unisalento.myfood.View.Decorator.Label.*;
import it.unisalento.myfood.View.Frame;
import it.unisalento.myfood.Listener.ArticoloPanelListener;
import it.unisalento.myfood.Listener.TextArticoloPanelFocusListener;
...
public class ArticoloPanel extends JPanel {
    ...
    private JLabel lblPezziDisponibiliArticolo = new JLabel();
    private JLabel lblQuantitaArticolo = new JLabel();
    private JLabel lblPrezzoArticolo = new JLabel();
    ...
    private ActionListener articoloListener;

    public ArticoloPanel(Frame frame) {
        articoloListener = new ArticoloPanelListener(frame, this);
        ...
        if (articoloIsProdotto) {
            ...
            lblPrezzoArticolo.setText("€ " + formatCurrency(prezzo));
            pnlPrezzo.add(lblPrezzoArticolo);
        } else {
            float sconto = (Float) articoloBusiness.getCampoSelectedArticolo(ArticoloBusiness.CAMPO.SCONTO);
            ...
            if (sconto > 0) {
                ...
                lblPrezzoArticolo.setText("€ " + formatCurrency(prezzoMenu));
            }
        }
        ...
        for (String s : directoryFoto) {
            ...
            WiewIconDecorator wiewIconDecorator = new WiewIconDecorator(originalIcon);
            foto.add(wiewIconDecorator.getImageIcon());
        }
        ...
        if (directoryFoto.size() == 1) {
            ...
            pnlFoto.removeAll();
            JLabel fotoAssente = new JLabel(new ImageIcon("src/immagini/icone/foto_prodotto_assente/foto_prodotto_assente.jpg"));
            pnlFoto.add(fotoAssente);
        }
        ...
        if (articoloIsProdotto) {
            textLabel = new TextLabel("Tipologia: " + (TipologiaProdottoBusiness.getInstance().getCampoTipologiaSelectedProdotto(TipologiaProdottoBusiness.CAMPO.NOME)));
        ...
        if (articoloIsProdotto) {
            ...
            textLabel = new TextLabel("Contiene:");
            DettagliTextLabelDecorator dettagliTextLabel = new DettagliTextLabelDecorator(textLabel);
            componenti.add(dettagliTextLabel.getLabel());

            JButton btnVisualizzaArticoli = new JButton("Visualizza Articoli nel Menu");
            btnVisualizzaArticoli.setPreferredSize(new Dimension(200, 30));
            btnVisualizzaArticoli.addActionListener(articoloListener);
            btnVisualizzaArticoli.setActionCommand(ArticoloPanelListener.VIEW_ARTICOLI_BTN);
            componenti.add(btnVisualizzaArticoli);
        ...
        // SOUTH
        int quantitaNelCarrello = loadQuantitaArticolo();
        int nPezziDisponibiliArticolo = (int) articoloBusiness.getCampoSelectedArticolo(ArticoloBusiness.CAMPO.DISPONIBILITA);

        JPanel firstRowSouth = new JPanel(new FlowLayout());
        lblPezziDisponibiliArticolo.setText("Pezzi disponibili: " + nPezziDisponibiliArticolo + "  ");
        lblQuantitaArticolo.setText("  Hai " + quantitaNelCarrello + " pezzi di questo articolo nel carrello");
        firstRowSouth.add(lblPezziDisponibiliArticolo);
        firstRowSouth.add(lblQuantitaArticolo);

        // Pulsanti
        if (nPezziDisponibiliArticolo != 0) {
            ...
            addToCart.setText("Disponibilità esaurita");
        }
    ...
    private JScrollPane loadCommenti() {
        ...
        /*
        Object[][] commenti = interazioneUtenteBusiness.getCommentiForSelectedArticolo();
        ...
        */
        ...
        /*scrollPaneCommenti = new JScrollPane(tableCommenti);
        JPanel pnlCommenti = new JPanel(new FlowLayout());
        pnlCommenti.add(scrollPaneCommenti);
         */
    ...
    private JPanel createUICommento() {
        JButton btnCommenti = new JButton("Visualizza tutti i commenti");
        btnCommenti.addActionListener(articoloListener);
        btnCommenti.setActionCommand(ArticoloPanelListener.VIEW_COMMENTI_BTN);
        btnCommenti.setPreferredSize(new Dimension(200, 30));
    ...
    public void updateUIQuantita(int quantita) {
        lblPezziDisponibiliArticolo.setText("Pezzi disponibili: " + articoloBusiness.getCampoSelectedArticolo(ArticoloBusiness.CAMPO.DISPONIBILITA) + "  ");
        lblQuantitaArticolo.setText("  Hai " + quantita + " pezzi di questo articolo nel carrello");
